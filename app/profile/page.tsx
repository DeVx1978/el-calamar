...
import { ChevronLeft, User, Mail, Calendar, Shield, Activity, Skull, Wallet, MapPin, Fingerprint, Database, Terminal, Star } from 'lucide-react';
...
export default function ReclutaProfile() {
  ...
  const [userStats, setUserStats] = useState({ ganadas: 0, perdidas: 0, total: 0, coins: 0 });

  useEffect(() => {
    async function fetchReclutaData() {
      // 1. Extraer datos de Autenticación
      const { data: { user } } = await supabase.auth.getUser();

      if (user) {
        // 2. Extraer datos de la tabla perfiles
        ...
        // 3. Extraer estadísticas de supervivencia
        ...
        if (predicciones) {
          ganadas = predicciones.filter(p => p.resultado === 'ganada').length;
          perdidas = predicciones.filter(p => p.resultado === 'perdida').length;
        }
        ...
        setUserData({
          ...
          nombre: perfil?.nombre || 'RECLUTA DESCONOCIDO',
          pais: perfil?.pais || 'ZONA NO IDENTIFICADA',
          recluta_num: perfil?.codigo_recluta || Math.floor(Math.random() * 8999) + 1000,
          // Aquí recibimos la URL de la foto del perfil si existe en la base de datos
        ...
        setUserStats({
          ...
          // ⚠️ Variable asignada correctamente a userStats
          coins: perfil?.pitchx_coins || 1500 
        });
      }
  ...
  // Cálculo de estatus vital
  const isDead = userStats.perdidas > 3; 
  ...
  if (loading) {
    return (
      <div style={{ height: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, fontFamily: 'monospace' }}>
        <motion.div animate={{ opacity: [0.3, 1, 0.3] }} transition={{ repeat: Infinity, duration: 1.2 }} style={{ color: '#00C853', letterSpacing: '6px', fontSize: 13, fontWeight: 900 }}>
          <Fingerprint size={40} style={{ margin: '0 auto 20px' }} />
  ...
  if (!userData) {
    return (
      <div style={{ height: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#FF0033', fontFamily: 'monospace' }}>
        ...
        <button onClick={() => router.push('/login')} style={{ marginTop: '30px', background: '#FF0033', color: '#fff', border: 'none', padding: '15px 30px', cursor: 'pointer', fontFamily: 'var(--font-display)' }}>IDENTIFICARSE</button>
      </div>
    );
  }
    ...
    <div className="profile-root">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; outline: none; }
        ...
        .id-card::before {
          ...
          background: ${isDead ? 'var(--red)' : 'var(--green)'};
          box-shadow: 0 0 20px ${isDead ? 'var(--red)' : 'var(--green)'};
        }
        ...
        .status-badge {
          ...
          background: ${isDead ? 'var(--red)' : 'var(--green)'}; color: #000;
          font-family: var(--font-mono); font-size: 9px; font-weight: 900;
          padding: 4px 12px; letter-spacing: 2px; box-shadow: 0 0 15px ${isDead ? 'var(--red)' : 'var(--green)'};
          z-index: 5;
        }
        ...
        /* CÓDIGO DE BARRAS GENERADO CON CSS */
        .barcode {
        ...
        /* ── PANEL DE ESTADÍSTICAS TÁCTICAS ── */
      ...
      <header className="top-bar">
        ...
        <div style={{ display: 'flex', gap: '15px', color: 'rgba(255,255,255,0.4)', fontFamily: 'var(--font-mono)', fontSize: '10px', alignItems: 'center' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Terminal size={12} /> SYS_DATA_LINKED</span>
        </div>
      </header>

      <main className="dossier-container">
        
        {/* ── COLUMNA 1: ID CARD (DATOS DE REGISTRO) ── */}
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
          <div className="id-card">
            
            {/* CONTENEDOR DEL AVATAR FOTOGRÁFICO */}
            <div className="avatar-box">
              {userData.avatar_url ? (
                <img src={userData.avatar_url} alt="Avatar Recluta" />
              ) : (
                <User size={50} color={isDead ? 'var(--red)' : 'rgba(255,255,255,0.2)'} />
              )}
              <div className="status-badge">{isDead ? 'ELIMINADO' : 'VIVO'}</div>
            </div>

            <div className="id-data-row" style={{ textAlign: 'center', marginBottom: '30px', border: 'none' }}>
              <span className="id-label">IDENTIFICACIÓN ASIGNADA</span>
              <span style={{ fontFamily: 'var(--font-impact)', fontSize: '32px', color: 'var(--red)', letterSpacing: '2px' }}>
                RECLUTA #{userData.recluta_num}
              </span>
            </div>
            ...
            <div className="id-data-row">
              <span className="id-label"><Mail size={10} className="inline mr-1" /> CREDENCIAL DE ACCESO (EMAIL)</span>
              <span className="id-value-mono">{userData.email}</span>
            </div>

            <div className="id-data-row">
              <span className="id-label"><MapPin size={10} className="inline mr-1" /> SECTOR (UBICACIÓN)</span>
              <span className="id-value">{userData.pais}</span>
            </div>

            <div className="id-data-row">
              <span className="id-label"><Calendar size={10} className="inline mr-1" /> FECHA DE INGRESO AL SISTEMA</span>
              <span className="id-value-mono">{new Date(userData.created_at).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' }).toUpperCase()}</span>
            </div>
            ...
            {/* Código de barras visual para estética PS5 */}
        ...
        {/* ── COLUMNA 2: ESTADÍSTICAS TÁCTICAS ── */}
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
          <div className="tactical-panel">
            
            <div className="vault-box">
              <div>
                <div className="vault-label">BÓVEDA PERSONAL (SALDO)</div>
                {/* ⚠️ CORRECCIÓN DE LA VARIABLE APLICADA AQUÍ */}
                <div className="vault-value">{userStats.coins.toLocaleString()} <span style={{fontSize: '14px', color: 'rgba(255,215,0,0.6)'}}>PX</span></div>
              </div>
            ...
            <div className="stats-grid">
              <div className="stat-box">
                <div className="stat-box-title"><Activity size={14} color="var(--green)" /> TASA DE SUPERVIVENCIA</div>
              ...
              <div className="stat-box">
                <div className="stat-box-title"><Star size={14} color="var(--green)" /> ACIERTOS TÁCTICOS</div>
              ...
              <div className="stat-box" style={{ borderColor: isDead ? 'rgba(255,0,51,0.3)' : 'rgba(255,255,255,0.05)' }}>
                <div className="stat-box-title"><Skull size={14} color="var(--red)" /> FALLOS CRÍTICOS</div>
            ...
            <button 
              onClick={() => router.push('/campo-de-batalla')}
              style={{ width: '100%', background: 'rgba(255,255,255,0.05)', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', padding: '20px', fontFamily: 'var(--font-display)', fontSize: '12px', letterSpacing: '2px', cursor: 'pointer', marginTop: '10px', transition: '0.3s' }}
              onMouseOver={(e) => { e.currentTarget.style.background = 'var(--red)'; e.currentTarget.style.borderColor = 'var(--red)'; }}
