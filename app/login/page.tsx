...
import React, { useState, useEffect, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Mail, Lock, Activity, X, ChevronRight, KeyRound } from 'lucide-react';
import { useRouter } from 'next/navigation';
...
const TrackingEye = ({ show, onClick, mousePos }: { show: boolean; onClick: () => void; mousePos: { x: number; y: number } }) => {
  ...
  useEffect(() => {
    ...
    const angle = Math.atan2(dy, dx);
    const distance = Math.sqrt(dx * dx + dy * dy);
    setPupilOffset({ x: Math.cos(angle) * Math.min(2, distance / 40), y: Math.sin(angle) * Math.min(2, distance / 40) });
  }, [mousePos]);
  ...
  return (
    <div className="auth-portal-container">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* --- EFECTO DE LUZ PERIMETRAL (SNAKE BORDER RED) --- */
        .card-wrapper {
          position: relative; z-index: 10; width: 100%; max-width: 400px;
          background: #000; border-radius: 8px; padding: 3px; overflow: hidden;
          box-shadow: 0 0 50px rgba(0, 0, 0, 1); display: flex; flex-direction: column;
        ...
        /* MATAR AUTOCOMPLETE BLANCO */
        .elite-input:-webkit-autofill, .elite-input:-webkit-autofill:hover, .elite-input:-webkit-autofill:focus {
          -webkit-text-fill-color: #fff !important;
          -webkit-box-shadow: 0 0 0px 1000px #000 inset !important;
          transition: background-color 5000s ease-in-out 0s;
        }

        .elite-input:focus { border-color: #FF0033 !important; box-shadow: 0 0 15px rgba(255,0,51,0.2) !important; }
        
        .eye-btn-tactical { 
          position: absolute; right: 12px; top: 50%; transform: translateY(-50%); 
          background: none !important; border: none !important; color: rgba(255,255,255,0.2); cursor: pointer; z-index: 10;
          transition: 0.3s; padding: 5px; display: flex; align-items: center; justify-content: center;
        }
        .eye-btn-tactical:hover { color: #fff; }
        ...
        .register-link-btn {
          ...
          cursor: pointer; transition: 0.3s; padding: 10px; margin: 0 auto; display: block;
        }
        .register-link-btn:hover { color: #fff !important; text-shadow: 0 0 10px #fff; }

        .system-message { padding: 12px; border-left: 3px solid #FF0033; background: rgba(255,0,51,0.1); color: #FF0033; font-size: 10px; font-weight: 900; text-align: center; margin-bottom: 15px; font-family: 'Syncopate'; }
      `}} />
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card-wrapper">
        <div className="form-content">
          <button type="button" className="close-btn" onClick={() => router.push('/')}>
            <X size={20} />
          </button>
          ...
          <div className="auth-header">
            <h1 className="auth-logo">ACCESO</h1>
            <p className="auth-subtitle">IDENTIFICACIÓN DE VETERANO</p>
          </div>

          <form onSubmit={handleLogin} autoComplete="off">
            <div className="input-group">
              <Mail size={14} className="input-icon" />
              <input type="email" className="elite-input" placeholder="CORREO DE RECLUTA" value={email} onChange={e => setEmail(e.target.value)} required />
            </div>

            <div className="input-group">
              <Lock size={14} className="input-icon" />
              <input type={showPassword ? "text" : "password"} className="elite-input" placeholder="CLAVE ENCRIPTADA" value={password} onChange={e => setPassword(e.target.value)} required />
              <TrackingEye show={showPassword} onClick={() => setShowPassword(!showPassword)} mousePos={mousePos} />
            </div>

            <AnimatePresence>
              {systemMessage && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="system-message">
                  {systemMessage}
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <button type="submit" className="submit-btn" disabled={isLoading}>
              {isLoading ? <Activity className="animate-spin mx-auto" size={20} /> : <span className="flex items-center justify-center gap-2">INGRESAR <ChevronRight size={18}/></span>}
            </button>
            
            <button type="button" onClick={() => router.push('/register')} className="register-link-btn">
              ¿NUEVO RECLUTA? SOLICITAR CONTRATO
            </button>
          </form>
        </div>
