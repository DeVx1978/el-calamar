...
useEffect(() => {
    const motorSimulacion = setInterval(() => {
      ...
      if (Math.random() > 0.9) {
        const id = Date.now();
        setNotifications(prev => [{ id, text: `RECLUTA HA INGRESADO`, type: 'entry' }, ...prev].slice(0, 3));
  ...
  return (
    <div className="calamar-root">
      ...
      <main className="hero-area">
        ...
        >
          ...
          <div className="panel-row">
            ...
            <span className="panel-val gold">${jackpotTotal.toLocaleString()} PX</span>
          </div>
          <div className="panel-row">
      ...
      <section style={{ 
        padding: '80px 0', 
        background: 'linear-gradient(to bottom, rgba(0,0,0,0.6), #000, rgba(0,0,0,0.8))', 
        ...
        <div style={{ textAlign: 'center', marginBottom: '60px' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(24px, 3vw, 36px)', fontWeight: 900, color: '#fff', textTransform: 'uppercase' }}>
            ZONA DE INTELIGENCIA <span style={{ color: THE_GAME.color, display: 'block', fontSize: '12px', letterSpacing: '8px', marginTop: '10px', fontFamily: 'var(--font-mono)' }}>ESPACIO OFICIAL DE PATROCINADORES</span>
          </h2>
        </div>

        <div className="hero-game-wrapper">
          {/* 🟢 ALTURA AUMENTADA PARA MOSTRAR LA COPA Y TÍTULO SIN CORTES (CONTAIN + NEGRO) */}
          <div style={{ width: '100%', height: 'clamp(400px, 65vh, 750px)', overflow: 'hidden', background: '#000' }}>
            <img 
              src={THE_GAME.image} 
              alt={THE_GAME.name} 
          ...
          {/* DESCRIPCIÓN CON ESTILO CALAMAR (BEBAS NEUE) */}
          <div style={{ marginTop: '40px', padding: '0 20px', textAlign: 'center' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
              <div style={{ padding: '15px', background: 'rgba(0,0,0,0.8)', borderRadius: '50%', border: `1px solid ${THE_GAME.color}`, boxShadow: `0 0 20px ${THE_GAME.color}40` }}>
                {THE_GAME.icon}
              </div>
            </div>
            <p className="bloody-text" style={{ fontSize: 'clamp(20px, 4vw, 36px)', color: '#fff', maxWidth: '1100px', margin: '0 auto', lineHeight: '1.2' }}>
              {THE_GAME.desc}
            </p>
          </div>
        </div>
      ...
      {/* 🟢 MODAL FLOTANTE CERO RESTRICCIONES (JUEGO SEGURO / TRANSMISIÓN) 🟢 */}
      <AnimatePresence>
        {isGameActive && (
          <motion.div 
            initial={{ opacity: 0 }} 
